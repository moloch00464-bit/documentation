...
## ⚠️ CRITICAL UPDATE: Language Parameter Issue

**Date:** 2026-01-12 (Post-initial implementation)

### **Issue Discovered**

During live testing on Termux, discovered that `termux-speech-to-text` **does NOT support `-l` language parameter**!

**Error:**
```
termux-speech-to-text: illegal option -l
```

### **Root Cause**

`termux-speech-to-text` is a thin wrapper around Android's native Speech Recognition API. Unlike `termux-tts-speak` (which has `-l` parameter), it **uses system-wide language settings** from Android!

### **Additional Commits**

**Commit:** `5bdb2be` - German voice test script (revealed the issue)
**Commit:** `e2ab56c` - Remove `-l` parameter and add setup documentation

### **Final Solution**

**Changed:** `moloch_3.0/moloch_io/voice.py`
- Removed `-l language` parameter from `listen()` method
- Updated to call `termux-speech-to-text` without parameters
- Added documentation pointing to Android settings

**Added:** `GERMAN_VOICE_SETUP.md`
- Complete guide for setting German in Android settings
- Device-specific instructions (Xiaomi, Samsung, Stock Android)
- Troubleshooting common issues
- Technical explanation of why no `-l` parameter exists

### **User Action Required**

To use German speech recognition, user must configure Android:

```
Settings → System → Language & Input
  → On-screen keyboard → Google Voice Typing
    → Languages → Deutsch (Deutschland) ✅
```

### **Impact on Review**

**Question for Opus 4.5:**

Should we provide an **automated check** or **warning message** when:
1. System language is German BUT
2. termux-speech-to-text recognizes English?

This would help users discover misconfigured Android Voice Typing settings automatically.

**Possible solution:**
```python
def check_voice_language():
    """Detect if STT language matches system language"""
    # Test STT with known German phrase
...
## ⚠️ CRITICAL UPDATE 2: Claude Model Name Fix

**Date:** 2026-01-12 (Post-API key fix)

### **Issue Discovered**

After fixing the API key issue (401 → 200 ✅), discovered that the Claude model name was outdated, causing 404 errors:
...
M.O.L.O.C.H. 3.0 config was using old model naming convention:
- **Old (WRONG):** `claude-sonnet-4-20250514`
- **Correct:** `claude-sonnet-4-5-20250929`

Claude model naming changed to include minor version (4-5 instead of 4).

### **Additional Commit**

**Commit:** `7d4f839` - Update to correct Claude model name

### **Final Solution**

**Changed:** `moloch_3.0/core/config.py`
- Line 19: Updated `CLAUDE_MODEL = "claude-sonnet-4-5-20250929"`
- Removed OpenAI API key validation (lines 200-209)
- Added comment explaining no more Whisper dependency

**Added:** `test_api_model.py`
- Imports model name from config
- Tests API with current model
- Clear success/error reporting
- Shows Claude's actual response

### **Testing Script**

User can now verify API + model work:
```bash
cd ~/documentation
bash update_moloch.sh
python test_api_model.py
```

**Expected output:**
```
✅ API & MODEL FUNKTIONIEREN!
🤖 Claude antwortet: MOLOCH LEBT!
```

### **Impact on Architecture**

This completes the migration to current Claude API:
- ✅ Correct model name (Sonnet 4.5)
- ✅ No OpenAI dependency
- ✅ Native Termux voice (no external API costs for STT)
- ✅ Valid API key configured
- ✅ Full voice + vision capabilities

**M.O.L.O.C.H. 3.0 is now production-ready! 🚀**
...
*Session ID: 6UkkI*
*Branch: claude/moloch-health-check-6UkkI*
*Last Updated: 2026-01-12 (Model fix completed)*
