...
Antworte immer im JSON Format."""
    ...
    def apply_fix(
        ...
        SAFEGUARD: Requires user approval before applying fix!
        ...
        Args:
            ...
            require_approval: If True, ask user before applying (DEFAULT: True)
        ...
        # SAFEGUARD: Show fix and ask for approval
        if require_approval:
            approved = self._ask_user_approval(fix, file_path)
            if not approved:
                self.logger.info("Fix rejected by user")
                return False
    ...
    def _ask_user_approval(self, fix: Dict, file_path: Optional[str] = None) -> bool:
        """
        Ask user for approval before applying fix

        Args:
            fix: Fix dict
            file_path: File to be modified (if any)

        Returns:
            True if approved, False otherwise
        """
        print("\n" + "="*70)
        print("🛠️  M.O.L.O.C.H. SELF-DEBUGGER: FIX VORSCHLAG")
        print("="*70)
        print(f"\n📋 DIAGNOSE:")
        print(f"   {fix.get('diagnosis', 'Unknown')}")
        print(f"\n💡 FIX BESCHREIBUNG:")
        print(f"   {fix.get('fix_description', 'Unknown')}")
        ...
        print(f"\n🛡️  VORBEUGUNG:")
        print(f"   {fix.get('preventive_measure', 'Unknown')}")
        print("\n" + "="*70)
        print("⚠️  WARNUNG: Dieser Fix wird automatisch ausgeführt!")
        print("   Backup wird erstellt, aber prüfe den Code genau!")
        print("="*70)

        # Ask for approval
        response = input("\n🤔 Fix anwenden? (y/N): ").strip().lower()

        if response in ['y', 'yes', 'ja', 'j']:
            print("✅ Fix wird angewendet...\n")
            return True
        else:
            print("❌ Fix abgelehnt\n")
            return False
