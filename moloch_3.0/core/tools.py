...
- web_search: Web durchsuchen (DuckDuckGo - Custom Tool)

CLAUDE NATIVE TOOLS:
- web_search_20250305: Claude's native web search (BESSER als Custom!)
"""
...
class FileTool:
    ...
    def write(self, path: str, content: str, backup: bool = True):
...
class WebTool:
    ...
    def search(self, query: str, max_results: int = 5):
        """Search web"""
        # Simple DuckDuckGo search via HTML scraping
        try:
            import requests
            ...
            if response.status_code == 200:
                # Very basic parsing - just return placeholder
                return [{
                    "title": f"Result for: {query}",
                    "url": "https://duckduckgo.com",
                    "snippet": "Use Claude native web_search for better results!"
                }]
...
def _tool_brain_load(tool_input: Dict, brain) -> Dict:
    ...
    # Use brain.load() to search
    results = brain.load(category, search_term if search_term else None)
...
# ═══════════════════════════════════════════════════════════════════════════════
# CLAUDE NATIVE WEB SEARCH (from M.O.L.O.C.H. 2.0)
# ═══════════════════════════════════════════════════════════════════════════════

def needs_web_search(user_input: str) -> bool:
    """
    Prüft ob Web Search gebraucht wird (from M.O.L.O.C.H. 2.0)

    Args:
        user_input: User's input text

    Returns:
        True if web search should be activated
    """
    triggers = [
        "such mal", "google", "hol dir", "recherchier",
        "was gibts neues", "aktuelle", "finde infos",
        "hol mir", "schau nach", "news", "suche nach",
        "was ist neu", "aktuell"
    ]
...
def get_claude_native_tools(enable_web_search: bool = False) -> List[Dict]:
    """
    Get Claude's native tools (web_search_20250305)

    Args:
        enable_web_search: Enable native web search

    Returns:
        List of native tool definitions
    """
    tools = []
