...
class VoiceIO:
    ...
    def listen(self, duration: int = 20, smart: bool = False) -> Optional[str]:
        ...
        """
        # SAFEGUARD: Check if Whisper API call is allowed
        guard = get_api_guard()
        allowed, reason = guard.can_call_whisper()

        if not allowed:
            print(f"\n🚨 WHISPER RATE LIMIT: {reason}")
            print("   Warte ein paar Minuten und versuch es nochmal!\n")
            return None
        ...
        # Step 3: Transcribe with Whisper (with safeguard recording)
    ...
    def _transcribe_whisper(self, duration: int = 20) -> Optional[str]:
        """
        ...
        SAFEGUARD: Records API call for cost tracking

        Args:
            duration: Audio duration in seconds (for cost tracking)
        ...
        try:
            ...
            # SAFEGUARD: Record successful API call
            guard = get_api_guard()
            guard.record_whisper_call(duration_seconds=duration)
