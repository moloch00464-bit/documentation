...
# MolochVolume - Lautstärke-Kontrolle (HOMESCREEN-FREUNDLICH!)
# Slider für Volume, speichert in config/volume.txt
...
CURRENT_VOL="70"
[ -f "$HOME/moloch/config/volume.txt" ] && CURRENT_VOL=$(cat "$HOME/moloch/config/volume.txt")
...
# Volume speichern
echo "$NEW_VOL" > "$HOME/moloch/config/volume.txt"

# System-Lautstärke setzen (alle Streams)
